...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->snpc; s->dnpc = (src1 + imm) & (~1);
    IFDEF(CONFIG_FTRACE, ftrace_func(s->pc, s->dnpc, s->isa.inst.val, imm)));
  ...
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, R(rd) = (SEXT((word_t)src1, 32) * SEXT((word_t)src2, 32)) >> 32);
...
int isa_exec_once(Decode *s) {
  ...
  IFDEF(CONFIG_ITRACE, add_iring_buf(s));
